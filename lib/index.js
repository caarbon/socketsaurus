 ...
 * @param  {Object} socketOptions   Options to use within socket connections
 */
function exhibitionist(io, Db, options, Collection, socketOptions) {
  ...
  var prep = socketOptions.prep || options.prep;
  var criteria;
  ...
  namespace.on('connection', function(socket) {
    ...
    socket.emit = function(action, payload) {
      var args = Array.prototype.slice.call(arguments);

      if (!socketOptions.filter || !socketOptions.filter(action, payload)) {
        return;
      }
...
function compare(input, criteria) {
  ...
  for (var key in criteria) {
    if (toString.call(criteria[key]) === '[object Object]') {
...
function socketsaurus(io, options) {
  ...
  return require('picu').function.curry(exhibitionist, io, new Reactive(options.oplogUrl), options);
}
socketsaurus.models = [];
